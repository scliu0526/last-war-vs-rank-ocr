...
目前已具備 .NET 10 WPF 基線、圖片選取／批次取消、候選列勾選與編輯、Open XML 活頁簿建立／安全更新、影像前處理、文字偵測、原始解析度裁切辨識、CTC 解碼與排名解析的本機 ONNX OCR 路徑、設定與隱私安全 Log。官方 PP-OCRv5 ONNX 模型已固定來源、授權與 SHA-256，並包含只套用於指揮官／同盟區域的韓文與泰文辨識模型；CPU 是預設模式，DirectML 可選擇相容顯示卡。目前已有一張中韓混合實際截圖的辨識證據；更多截圖、泰文、GPU 一致性、乾淨機離線啟動與 Windows 10 仍須依驗證矩陣另行確認。在沒有模型包時，仍可用同名 `.txt` 側錄預覽流程。
