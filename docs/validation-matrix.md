...
| #6 | ONNX Runtime CPU pipeline, verified model manifest/hash/licence gates, PP-OCRv5 preprocessing/shape handling, parser and sidecar fallback | Unit tests, fake-runtime OCR integration tests, official model hashes, one local model smoke probe (sessions load; sample produced 0 candidates) | CPU OCR accuracy and private screenshot ground truth remain unverified; parser/detection tuning still required |
