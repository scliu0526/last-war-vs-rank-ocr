...
| #6 | ONNX Runtime CPU pipeline, verified model manifest/hash/licence gates, PP-OCRv5 preprocessing/shape handling, YAML dictionary normalization, parser and sidecar fallback, rank-column retry for merged detection boxes, annotated ranking-region ROI/column filtering, card-row grouping, multi-width numeral-band retry, punctuation-tolerant numeric parsing | Unit tests, fake-runtime OCR integration tests, official model hashes, and local PP-OCRv5 CPU smoke against the provided ground truth (7 candidates emitted; rank 1 and `GBgogogo` recovered, but score and multilingual fields remain inaccurate) | Exact rows 1–7 are not yet reproduced; the current primary dictionary has only two Korean entries and no Thai entries, so official language-specific recognition models remain required |
