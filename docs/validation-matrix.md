...
| #6 | ONNX Runtime CPU pipeline, verified model manifest/hash/licence gates, PP-OCRv5 preprocessing/shape handling, parser and sidecar fallback | Unit tests, fake-runtime OCR integration tests, official model hashes, one local PP-OCRv5 CPU smoke probe (sample produced 8 candidates; rank 1 score `72138569` matched the screenshot) | Full row/name/alliance accuracy and private screenshot ground truth remain unverified; the sample's alliance text still needs review |
