...
| #6 | ONNX Runtime CPU pipeline, verified model manifest/hash/licence gates, PP-OCRv5 preprocessing/shape handling, parser and sidecar fallback, rank-column retry for merged detection boxes, annotated ranking-region ROI/column filtering, proportional row grouping, multi-width numeral-band retry, punctuation-tolerant numeric parsing | Unit tests, fake-runtime OCR integration tests, official model hashes, and current local PP-OCRv5 CPU smoke probe (sessions execute and emit 7 candidates, but the expected rank-1/`72,138,569` row is still misread; probe ~1m43s) | Full row/name/alliance accuracy and private screenshot ground truth remain unverified; DB postprocessing, rank association and detection tuning remain required |
...
| #13 | Workflow, batch, workbook, security-gate and CI automation | Release build, 58 tests, WPF `App.OnStartup` smoke test, security script, Windows CI definition | Published EXE/ZIP process launch, real Windows 11 CPU/GPU/model run, clean-machine offline launch; Windows 10, Japanese and Thai remain unverified |
