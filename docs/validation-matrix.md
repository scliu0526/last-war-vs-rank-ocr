...
| #6 | ONNX Runtime CPU pipeline, verified model manifest/hash/licence gates, PP-OCRv5 preprocessing/shape handling, parser and sidecar fallback, rank-column retry for merged detection boxes, annotated ranking-region ROI filtering | Unit tests, fake-runtime OCR integration tests, official model hashes, and a current local PP-OCRv5 CPU smoke probe (sessions execute, but the annotated sample still emitted `0` confirmed candidates) | Full row/name/alliance accuracy and private screenshot ground truth remain unverified; DB postprocessing and detection tuning remain required |
