...
| #6 | ONNX Runtime CPU pipeline, verified model manifest/hash/licence gates, PP-OCRv5 preprocessing/shape handling, parser and sidecar fallback, rank-column retry for merged detection boxes, annotated ranking-region ROI filtering, multi-width numeral-band retry | Unit tests, fake-runtime OCR integration tests, official model hashes, and current local PP-OCRv5 CPU smoke probes (sessions execute, but the annotated sample still emits `0` confirmed candidates after ROI and multi-width rank retries) | Full row/name/alliance accuracy and private screenshot ground truth remain unverified; DB postprocessing and detection tuning remain required |
...
| #13 | Workflow, batch, workbook, security-gate and CI automation | Release build, 55 tests, WPF `App.OnStartup` smoke test, security script, Windows CI definition | Published EXE/ZIP process launch, real Windows 11 CPU/GPU/model run, clean-machine offline launch; Windows 10, Japanese and Thai remain unverified |
