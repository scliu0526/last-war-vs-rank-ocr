...
| --- | --- | --- | --- |
| #4 | Workbook creation, selective update, backup rotation, unknown-workbook rejection, invalid-target handling and lock-safe replacement/retry | `dotnet test RankLens.slnx --configuration Release` | No known local code gap; real Excel lock/reopen session remains unperformed; issue is closed |
...
| #7 | Category/parser scaffolding, supported aspect-ratio validation, row bounds and source geometry; exact synthetic Japanese/Thai/Korean text preservation; installed Japanese-capable primary, Korean and Thai models run on a deidentified tensor | Parser, validator, model-capability and synthetic multilingual tests | Real six-sample classification and per-row ground truth; real Japanese/Thai screenshots remain unavailable and unverified |
| #8 | Candidate grid, selection policy, conflict controls, category correction, source switching and zoom | WPF tests cover date, edit, originally checked/unchecked confirmation, category propagation, no-alliance confirmation, source switching and conflict buttons; user confirmed unchecking an initially selected row works | Close-dialog Yes/No interaction and final user recheck of the originally-unchecked row on the rebuilt executable |
...
| #13 | Workflow, 100-image batch, workbook safety, WPF review paths, model capability, security gate and CI automation | Release build, 83 tests, WPF `App.OnStartup` test, security script, freshly extracted ZIP startup smoke, and current-machine CPU/Intel/RTX evidence | Clean-machine offline launch; Windows 10 and real Japanese/Thai screenshots remain unverified |
