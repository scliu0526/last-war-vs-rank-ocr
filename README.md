...
The current baseline includes the .NET 10 WPF shell, fixed-candidate workflow, Open XML workbook creation/update, image discovery, review controls, cancellable batch processing, and a local ONNX OCR path (image preprocessing, text detection, original-resolution crop recognition, CTC decoding, and ranking parsing). The release workflow uses the SHA-256-pinned official PP-OCRv5 ONNX bundle described in `models/manifest.json`, including Korean and Thai recognition variants applied only to commander/alliance regions. One provided mixed Chinese/Korean screenshot has been exercised; broader screenshot coverage, Thai accuracy, clean-machine launch, and hardware accuracy remain explicit validation boundaries. If no model bundle is installed, the review flow supports deterministic offline preview through a same-name `.txt` sidecar next to each image (`rank<TAB>commander<TAB>alliance<TAB>score`).
